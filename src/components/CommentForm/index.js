...
const CommentForm = ({post_id, type}) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <textarea id="comment" name="comment" value={comment} onChange={handleChange} rows="2" className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Leave a comment..."></textarea>
            {type === 'meal' && 
            <div className="relative pt-1">
                <label for="customRange3" className="form-label">Rating: {rating}/10</label>
                <input onChange={ratingChange} value={rating} type="range" className="form-range appearance-none w-full h-6 p-0 bg-gray-300 rounded-md focus:outline-none focus:ring-0 focus:shadow-none" min="0" max="10" step="1"/>
            </div>
            }
